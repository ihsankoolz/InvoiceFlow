...
class PaymentGRPCClient:
    ...
    async def convert_escrow(
        self, investor_id: int, invoice_token: str, idempotency_key: str
    ) -> dict:
        """Call ConvertEscrowToLoan RPC."""
        from proto import payment_pb2

        stub = await self._get_stub()
        response = await stub.ConvertEscrowToLoan(
            payment_pb2.ConvertEscrowRequest(
    ...
    async def create_loan(
        ...
        stub = await self._get_stub()
        response = await stub.CreateLoan(
            payment_pb2.CreateLoanRequest(
                invoice_token=invoice_token,
                investor_id=investor_id,
                seller_id=seller_id,
    ...
    async def release_funds(self, seller_id: int, amount: float, invoice_token: str) -> dict:
        ...
        stub = await self._get_stub()
        response = await stub.ReleaseFundsToSeller(
            payment_pb2.ReleaseFundsRequest(
                seller_id=seller_id,
                amount=str(amount),
                invoice_token=invoice_token,
    ...
    async def update_loan_status(self, loan_id: str, status: str, grace_end: str = "") -> dict:
        ...
        stub = await self._get_stub()
        response = await stub.UpdateLoanStatus(
    ...
    async def credit_wallet(self, user_id: int, amount: float) -> dict:
        ...
        stub = await self._get_stub()
        response = await stub.CreditWallet(
            payment_pb2.CreditWalletRequest(
                user_id=user_id,
                amount=str(amount),
    ...
    async def release_escrow(
        ...
        stub = await self._get_stub()
        response = await stub.ReleaseEscrow(
            payment_pb2.ReleaseEscrowRequest(
                investor_id=investor_id,
                invoice_token=invoice_token,
                idempotency_key=idempotency_key,
            )
