...
function fadeUp(visible, delay = 0) {
  if (visible) return { animation: 'repayFadeUp 600ms ease both', animationDelay: `${delay}ms` }
...
export default function RepaymentsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const res = await api.get(`/loans?investor_id=${user.sub}`)
        const data = res.data?.loans || res.data || []
        setLoans(Array.isArray(data) ? data : [])
      } catch (e) {
        setError(e.response?.data?.detail || e.message || 'Failed to load repayments.')
      } finally {
        setLoading(false)
      }
    }
    ...
    <AppLayout>
      ...
      <div ref={headerRef} className="bg-teal px-8 py-10" style={fadeUp(headerInView, 0)}>
        <div className="max-w-6xl mx-auto">
          <h1 className="font-['Lato'] font-semibold text-[42px] text-white leading-tight">My Repayments</h1>
          <p className="font-['Lato'] text-white/60 text-sm mt-1">Track and manage your loan repayments</p>
        </div>
      </div>
        ...
        <div ref={gridRef}>
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="font-['Lato'] font-medium text-ink/40 mb-1">No repayments found</p>
              <p className="font-['Lato'] text-sm text-ink/30">
                {activeTab === 'ALL' ? "You don't have any loans yet." : `No loans with status "${activeTab}".`}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
